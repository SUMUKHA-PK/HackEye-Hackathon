...
def get_items(filepath,savefolder):
    ...
    for recipe_no,recipe in enumerate(recipes):
        ...
        recipe_table.append((recipe_items,recipe_no,recipe_names[recipe_no]))
    for item,recipe in zip(recipes[:10],recipe_table[:10]):
        print(item,recipe,sep="\n")
        print('-'*90)
    pd.DataFrame(recipe_table,columns=["recipe","recipe_number","recipe_name"]).to_csv(savefolder+"/recipe_table.csv",index=False)
